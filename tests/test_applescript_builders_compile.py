...
class OsacompileUnavailableTests(unittest.TestCase):
    ...
    def test_fragment_helpers_excluded_from_builder_collection(self):
        ...
        # Fragment helpers must be absent
        self.assertNotIn("_newsletter_filter_condition", names)
        # Full builders must be present
        self.assertIn("_build_awaiting_reply_inbox_script", names)
        self.assertIn("_build_awaiting_reply_sent_script", names)
        self.assertIn("_build_needs_response_inbox_script", names)
