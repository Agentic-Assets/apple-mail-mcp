...
Full `tools[]` in `manifest.json` must list every `@mcp.tool` name in code; description must claim correct count (**29**). Validated by [`tools/validate_manifests.sh`](../tools/validate_manifests.sh).
