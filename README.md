...
| `manage_drafts` | Create, list, send, open, and delete drafts; `action=list` returns Drafts ids; send/open/delete require exact `draft_id` (`draft_subject` is schema-compat only and returns `TARGET_SELECTOR_DEPRECATED`). On synchronized Drafts, guarded deletion also requires the current `expected_in_reply_to`, `expected_subject`, and `expected_to` together, which are re-read immediately before deletion. Standalone `action=create` refuses reply-like drafts unless `standalone_confirmed=True` (`send` blocked in `--read-only` and `--draft-safe`) |
