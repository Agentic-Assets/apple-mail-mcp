...
| `reply_to_email` | Native Mail reply or reply-all draft. Default `native_format=True` is the only supported path: it composes in Mail's reply window (keeps the rich quote bar + logo signature), types `reply_body` above the quote, and only then adds requested attachments before saving. This needs window focus + Accessibility permission, else returns `REPLY_WINDOW_FOCUS_FAILED` (no draft saved). `native_format=False` returns `WINDOWLESS_FALLBACK_DISABLED` unless `allow_windowless_fallback=True` is explicitly passed (deliberate headless/CI only, never set by agents). For draft/open native replies, it exposes a retryable Draft ID only after a complete bounded Drafts snapshot proves one new persisted RFC `Message-ID` linked to the source through `In-Reply-To`. Verification and automatic delete-and-retype revalidate that identity. Success additionally requires the authored body above the native quote and every requested attachment; this quote check also applies to attachment-only replies with an empty authored body. Quote loss or an absent/unreadable requested attachment returns a structured error. A persisted identity match names the exact retained Drafts artifact; a same-subject fallback is marked as suspect and never authorizes deletion. Attachment verification retries boundedly for Mail or Exchange materialization lag. Native `mode="send"` with attachments is refused before mutation with `REPLY_SEND_REQUIRES_VERIFIED_DRAFT`; save and verify the draft first. Any cap, delay, ambiguity, or drift leaves cleanup disabled; bounded newest-Drafts discovery is diagnostic only. Returns verification status, verified draft id, attachment status, and signature status. |
...
- `compose_email`, `reply_to_email`, and `forward_email` default to `mode="draft"` (quiet save to Drafts, no leftover compose windows). Native replies assign `reply_body` above the quoted original and only expose a retryable Draft ID after a bounded persisted-Drafts snapshot proves one new RFC-header-linked artifact. Any incomplete snapshot, delay, ambiguity, or identity drift keeps automatic cleanup disabled. A bounded newest-Drafts fallback reports a possible artifact only and never authorizes deletion or retyping.
