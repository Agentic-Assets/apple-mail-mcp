...
Navigation hub for **apple-mail-mcp**: one Python MCP server (**28 tools**, **315+ tests**, `fastmcp>=3.1.0,<4`) shipped as PyPI package (`mcp-apple-mail`), Claude Code plugin (`plugin/`), and Claude Desktop `.mcpb` (`apple-mail-mcpb/`). Marketplace entry: `.claude-plugin/marketplace.json`.
...
.venv/bin/pytest tests/                    # 315+ tests
