...
**Next action:** adversarial review fixes, version bump to 3.11.1 + `bash tools/gates/dev-check.sh release`, a draft-mode-only live-verification pass, then ship per the orchestration record. No PR (not authorized for this repo); merge needs Cayman's explicit phrase.
