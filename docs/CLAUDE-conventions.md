...
**Reply/forward targeting:** pass `message_id` from `search_emails`, `list_inbox_emails`, or `get_email_by_id`. `subject_keyword` is schema-compatible only and returns `TARGET_SELECTOR_DEPRECATED`; run discovery first. `reply_to_email` defaults to the native reply window (`native_format=True`), the only supported path for normal agent use, so drafts keep Mail's rich quote bar and logo signature; this needs the Mail window to take focus and **Accessibility permission for the host process** (System Events keystrokes, sent in small focus-guarded chunks rather than one keystroke of the whole body). Requested attachments are added only after typing succeeds and immediately before save so the object-model mutation cannot disrupt the typer's native reply cursor. Native `mode="send"` with attachments returns `REPLY_SEND_REQUIRES_VERIFIED_DRAFT` before Mail mutation; use `mode="draft"`, inspect the verified artifact, and send that draft only after approval. If the window cannot be focused before typing starts, the tool returns `REPLY_WINDOW_FOCUS_FAILED` (or `REPLY_SUBJECT_GUARD_MISMATCH` when a compose window opened but its title never matched) without saving; if focus is lost partway through a chunked typing pass, it returns `REPLY_BODY_TYPING_INTERRUPTED` and discards the partially typed compose window instead of leaving a partial draft. Retry any of these with Mail visible and not being clicked. Do not switch to `native_format=False`, which is gated (`WINDOWLESS_FALLBACK_DISABLED`) and requires `allow_windowless_fallback=True` for the windowless object-model path reserved for deliberate headless/CI only (agents must never set it). After a native draft/open reply saves, the plugin can expose an exact Drafts target only when a complete bounded before/after snapshot proves exactly one new persisted message and its RFC `Message-ID` plus `In-Reply-To` link it to the source. The full saved body is compared above the quote case-sensitively, not as a first-line needle, and success additionally requires the native quote plus every requested attachment. The quote requirement also covers attachment-only replies with an empty authored body, and attachment verification retries boundedly for Mail or Exchange materialization lag. Quote loss returns `REPLY_QUOTED_ORIGINAL_MISSING`; a missing or unreadable requested attachment returns `REPLY_DRAFT_ATTACHMENT_VERIFICATION_FAILED`. When persisted identity was revalidated, the error names the exact retained Drafts artifact. Otherwise, a same-subject fallback id is marked as suspect and the remediation explicitly forbids automatic deletion. Neither failure triggers automatic deletion or retyping. One automatic delete-and-retype retry remains limited to body-placement failures after the same persisted identity is revalidated at the exact numeric Drafts ID. Cap limits, indexing delay, ambiguity, malformed headers, or ID drift disable automatic cleanup; bounded newest-Drafts fallback can identify a suspect artifact but never deletes it. `body_html` is ignored on replies for compatibility. Do not use standalone draft creators (`compose_email`, `create_rich_email_draft`, or `manage_drafts(action="create")`) to answer existing mail: they create standalone messages with no quoted original thread. These paths refuse reply-like `Re:` / `Fwd:` subjects or quoted-thread bodies unless the caller explicitly passes `standalone_confirmed=True`. `manage_drafts(action="create", in_reply_to=...)` additionally refuses up front with `CREATE_CANNOT_THREAD`: create has no header property to set In-Reply-To/References, so `in_reply_to` is honored only by `action="find"`.
