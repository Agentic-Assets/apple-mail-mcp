...
> **Historical:** Anti-patterns in this report (e.g. `search_emails` on Drafts for verify/find) are superseded by `verify_draft`, `manage_drafts(action="list")`, and ID-first guidance in [`docs/CLAUDE-conventions.md`](../CLAUDE-conventions.md). Use [`docs/AGENT_LIVE_TESTING.md`](../AGENT_LIVE_TESTING.md) for current live-verification procedures.
