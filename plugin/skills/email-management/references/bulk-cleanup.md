...
6. **Empty Trash is the nuclear option.** Run `manage_trash(action="empty_trash", confirm_empty=True)` only after explicit user confirmation.
...
3. Verify, then `manage_trash(action="empty_trash", confirm_empty=True)` if appropriate.
