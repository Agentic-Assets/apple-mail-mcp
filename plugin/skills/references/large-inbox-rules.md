...
|------|------|-------------|
| `UNBOUNDED_SCAN_REQUIRED` | Discovery tool called without `recent_days` / `max_emails` bounds | Pass bounded `recent_days` / `max_emails`, or call `full_inbox_export` if a full walk is truly required |
