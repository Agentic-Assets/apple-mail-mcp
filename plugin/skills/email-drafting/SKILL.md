...
|-----------|------|-------|
| New outbound mail | `compose_email` | Standalone only; default `mode="draft"`; use `mode="open"` only for explicit saved-open review. Attachments require draft/open plus immediate strict recipient, subject, saved-body, filename/count, and readability verification before human review and any later send. A numeric Draft ID is a locator, not durable identity. |
...
`verify_drafts(draft_ids=[...])`. A marker-bound attachment success with no
locator is ready for review but not a later lifecycle target: inspect it in the
open Mail window or wait for a separately re-resolved exact id. Use bounded
