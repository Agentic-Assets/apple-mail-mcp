...
2. `get_statistics(scope="account_overview", days_back=2)` for a quick load sample, then use `get_statistics(scope="mailbox_breakdown", mailbox="...")` only **per folder under review** — omitting `mailbox` on `mailbox_breakdown` scopes to the default Inbox in code, so always pass the folder name when assessing non-Inbox clutter. Note: `get_statistics(scope="account_overview", days_back=0)` now requires `allow_full_scan=True` (v3.1.9) — keep `days_back` bounded unless the user explicitly opts into a full scan.
