...
2. `get_statistics(scope="account_overview", days_back=2)` for a quick load sample, then use `get_statistics(scope="mailbox_breakdown", mailbox="...")` only **per folder under review** — omitting `mailbox` on `mailbox_breakdown` scopes to the default Inbox in code, so always pass the folder name when assessing non-Inbox clutter. Note (v3.2.0): `get_statistics(scope="account_overview", days_back=0)` is refused with `code: UNBOUNDED_SCAN_REQUIRED`. Keep `days_back` bounded; for a true full sweep call `full_inbox_export` (slow) and feed those results back through analytics.
