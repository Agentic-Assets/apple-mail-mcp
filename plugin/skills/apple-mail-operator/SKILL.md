...
- **Never verify drafts with `search_emails`.** Drafts are unsent `outgoing message` objects with a null received date, so the date-filtered search is both slow and silently drops them. Use the `Draft ID` returned by `reply_to_email` with `get_email_by_id(message_id=..., mailbox="Drafts")` for exact verification. If no id is available, use `manage_drafts(action="list", subject_contains="...")` (bounded, newest-first). `reply_to_email` also verifies its exact saved Draft ID before reporting success for draft/open modes.
