...
- Abort the draft when a discovery row shows `has_draft=true` (a matching draft already exists, trustworthy even from a truncated scan) or when `was_replied_to=true` with no matching draft; treat `has_draft=null` as unknown (scan skipped, errored, or the bounded 50-draft-per-account scan truncated without a match; see `draft_scan.truncated` / `draft_scan.total`) and fall back to a manual draft check. `has_draft=false` means the scan completed with no match. Override only when the user explicitly says "redraft", "include already-replied", or "include drafted".
